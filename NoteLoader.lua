...
function NoteLoader:__init(opt)
	...
	self.go_offset_token = #self.idx2offset+1
	self.go_note_token = 129
	...
	self.bnotes = {}
	self.bnotes.input = torch.Tensor(self.seq_length, self.batch_size):fill(self.go_note_token)
	self.bnotes.target = torch.Tensor(self.seq_length, self.batch_size, 1)
	
	self.boffsets = {}
	self.boffsets.input = torch.Tensor(self.seq_length, self.batch_size):zero():fill(self.go_offset_token)
	self.boffsets.target = torch.Tensor(self.seq_length, self.batch_size, 1)
...
function NoteLoader:nextBatch()
	...
	if self.sample then
		...
		self.current_batch = 1--(self.current_batch % self:getNumberOfBatches()) + 1
